 ...
 * @invite habbostoryfr
 * @website https://www.firebrandvisual.fr/
 * @source https://github.com/Firemansing/FriendNotifications-betterdiscord-FR/
 * @updateUrl https://github.com/Firemansing/FriendNotifications-betterdiscord-FR/FriendNotifications.plugin.js
 */
...
module.exports = (_ => {
	...
	return !window.BDFDB_Global || (!window.BDFDB_Global.loaded && !window.BDFDB_Global.started) ? class {
		...
		getDescription () {return `Le plugin Library nécessaire pour ${this.name} est manquant. Ouvrez les paramètres du plugin pour le télécharger. \n\n${this.description}`;}
		...
		downloadLibrary () {
			BdApi.Net.fetch("https://mwittrien.github.io/BetterDiscordAddons/Library/0BDFDB.plugin.js").then(r => {
				...
				else return require("fs").writeFile(require("path").join(BdApi.Plugins.folder, "0BDFDB.plugin.js"), b, _ => BdApi.showToast("Téléchargement de la bibliothèque BDFDB terminé", {type: "success"}));
			}).catch(error => {
				BdApi.alert("Erreur", "Impossible de télécharger le plugin Bibliothèque BDFDB. Réessayez plus tard ou téléchargez-le manuellement depuis GitHub: https://mwittrien.github.io/downloader/?library");
			});
		}
		...
		load () {
			...
			if (!window.BDFDB_Global.downloadModal) {
				...
				BdApi.showConfirmationModal("Bibliothèque manquante", `Le plugin Bibliothèque nécessaire pour ${this.name} est manquant. Veuillez cliquer sur "Télécharger maintenant" pour l'installer.`, {
		...
		getSettingsPanel () {
			...
			template.innerHTML = `<div style="color: var(--header-primary); font-size: 16px; font-weight: 300; white-space: pre; line-height: 22px;">Le plugin Bibliothèque nécessaire pour ${this.name} est manquant.\nVeuillez cliquer sur <a style="font-weight: 500;">Télécharger maintenant</a> pour l'installer.</div>`;
			...
			render() {
				return BDFDB.ReactUtils.createElement("div", {
					...
					children: BDFDB.ReactUtils.createElement("div", {
						...
						children: BDFDB.LanguageUtils.LanguageStringsFormat("AMIS_EN_LIGNE_EN_TÊTE", this.props.amount),
		...
		return class FriendNotifications extends Plugin {
			onLoad () {
				...
				this.defaults = {
					general: {
						addOnlineCount:			{value: true, 	description: "Ajoute un compteur d'amis en ligne à la liste des serveurs (Cliquez pour ouvrir le journal des temps)"},
						showDiscriminator:		{value: false, 	description: "Ajoute le discriminant de l'utilisateur"},
						showTimestamp:			{value: false, 	description: "Ajoute le timestamp"},
						muteOnDND:			{value: false, 	description: "Ne vous notifie pas lorsque vous êtes en mode Ne pas déranger"},
						openOnClick:			{value: false, 	description: "Ouvre le message direct lorsque vous cliquez sur une notification"}
					},
					notificationStrings: {
						online: 			{value: "$user a changé de statut en '$status'"},
						idle: 				{value: "$user a changé de statut en '$status'"},
						dnd: 				{value: "$user a changé de statut en '$status'"},
						playing: 			{value: "$user a commencé à jouer à '$game'"},
						listening: 			{value: "$user a commencé à écouter '$song'"},
						streaming: 			{value: "$user a commencé à diffuser en continu '$game'"},
						screensharing: 			{value: "$user a commencé à partager son écran"},
						offline: 			{value: "$user a changé de statut en '$status'"},
						login: 				{value: "$user vient de se connecter '$status'"},
						custom: 			{value: "$user a changé de statut en '$custom'"}
					},
					notificationSounds: {},
					dates: {
						logDate:			{value: {}, 			description: "Horodatage du journal"},
					},
					amounts: {
						toastTime:			{value: 5, 	min: 1,		description: "Durée d'affichage d'une notification toast à l'écran : "},
						checkInterval:			{value: 10, 	min: 5,		description: "Vérifie les utilisateurs toutes les X secondes : "}
					}
				};
			...
			getSettingsPanel (collapseStates = {}) {
				...
				let successSavedAudio = (type, parsedUrl, parsedData) => {
					if (parsedUrl && parsedData) BDFDB.NotificationUtils.toast(`Le son a été enregistré avec succès.`, {type: "success"});
				...
				let createUserList = (users, type, title) => {
					...
					items.push(BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Flex, {
						...
						children: [
							"Cliquez sur une option pour basculer",
					...
					if ("Notification" in window) items.push(BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Flex, {
						className: BDFDB.disCNS.settingsrowtitle + BDFDB.disCNS.settingsrowtitledefault + BDFDB.disCN.cursordefault,
						children: [
							"Clic droit sur une option pour basculer",
							BDFDB.ReactUtils.createElement("span", {
								className: BDFDB.disCN._friendnotificationstypelabel,
					...
					items.push(BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Flex, {
						...
						style: {marginTop: 6},
						children: [
							"Cliquez sur un en-tête d'option pour basculer",
							BDFDB.ReactUtils.createElement("span", {
								className: BDFDB.disCN._friendnotificationstypelabel,
								style: {backgroundColor: "var(--bdfdb-blurple)"},
								children: "Toast"
					...
					if ("Notification" in window) items.push(BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Flex, {
						className: BDFDB.disCNS.settingsrowtitle + BDFDB.disCNS.settingsrowtitledefault + BDFDB.disCN.cursordefault,
						children: [
							"Clic droit sur un en-tête d'option pour basculer",
							BDFDB.ReactUtils.createElement("span", {
								className: BDFDB.disCN._friendnotificationstypelabel,
								style: {backgroundColor: "var(--status-positive)"},
								children: "Bureau"
					...
					items.push(BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Flex, {
						className: BDFDB.disCNS.settingsrowtitle + BDFDB.disCNS.settingsrowtitledefault + BDFDB.disCN.cursordefault,
						style: {marginTop: 6},
						children: "Cliquez sur un avatar pour basculer entre activé/désactivé"
					}));
					if ("Notification" in window) items.push(BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Flex, {
						className: BDFDB.disCNS.settingsrowtitle + BDFDB.disCNS.settingsrowtitledefault + BDFDB.disCN.cursordefault,
						children: [
							"Clic droit sur un avatar pour basculer entre toutes les options",
							BDFDB.ReactUtils.createElement("span", {
								className: BDFDB.disCN._friendnotificationstypelabel,
								style: {backgroundColor: "var(--bdfdb-blurple)"},
								children: "Toast"
				...
				return settingsPanel = BDFDB.PluginUtils.createSettingsPanel(this, {
					...
					children: _ => {
						...
						settingsItems.push(BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.CollapseContainer, {
    title: "Paramètres par défaut pour les nouveaux utilisateurs",
    collapseStates: collapseStates,
...
let strangerId = "";
settingsItems.push(BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.CollapseContainer, {
    title: "Ajouter un nouvel inconnu",
    collapseStates: collapseStates,
        ...
        children: [
            BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Flex.Child, {
                children: BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.TextInput, {
                    placeholder: "utilisateur (id, nom du compte ou nom#discriminant)",
                    value: "",
            ...
            BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Button, {
                onClick: _ => {
                    ...
                    if (userId == BDFDB.UserUtils.me.id) BDFDB.NotificationUtils.toast("Sérieusement, vous essayez de vous observer vous-même ?", {type: "danger"});
                    else if (friendIds.includes(userId)) BDFDB.NotificationUtils.toast("L'utilisateur est déjà un de vos amis, veuillez utiliser la zone 'Liste d'amis' pour les configurer", {type: "danger"});
                    else if (observed.strangers[userId]) BDFDB.NotificationUtils.toast("L'utilisateur est déjà observé comme 'Inconnu'", {type: "danger"});
                    else {
                        ...
                        if (user) {
                            if (user.id == BDFDB.UserUtils.me.id) BDFDB.NotificationUtils.toast("Sérieusement, vous essayez de vous observer vous-même ?", {type: "danger"});
                            else {
                        ...
                        else BDFDB.NotificationUtils.toast("Veuillez entrer un ID ou un nom de compte valide d'un utilisateur qui a été chargé dans votre client", {type: "danger"});
                    }
                },
...
settingsItems.push(BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.CollapseContainer, {
    ...
    children: [BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Flex, {
        ...
        children: BDFDB.ReactUtils.createElement("div", {
            className: BDFDB.disCNS.settingsrowtitle + BDFDB.disCNS.settingsrowtitledefault + BDFDB.disCN.cursordefault,
            children: [
                "Vous permet de configurer vos propres chaînes de messages pour les différents statuts.",
                [
                    ["$user", " est le substitut du nom d'utilisateur"],
                    ["$nick", " pour le surnom de l'ami (par défaut $user s'il n'est pas utilisé)"],
                    ["$status", " pour le nom du statut"],
                    ["$statusOld", " pour l'ancien nom du statut"],
                    ["$custom", " pour le statut personnalisé"],
                    ["$game", " pour le nom du jeu"],
                    ["$song", " pour le nom de la chanson"],
                    ["$artist", " pour le nom de l'artiste de la chanson"]
        ...
        BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Flex, {
            ...
            children: [
                BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.SettingsLabel, {
                    label: `${key.split(/(desktop)|(toast)/).filter(n => n).map(n => BDFDB.StringUtils.upperCaseFirstChar(n)).join("-")} Son de notification`,
                }),
        ...
        BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Flex, {
            ...
            children: [
                ...
                BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.SettingsItem, {
                    ...
                    mini: true,
                    basis: "80%",
                    placeholder: "URL du son de notification",
                    value: this.settings.notificationSounds[key].url,
                    onChange: value => {
                        this.settings.notificationSounds[key].url = value;
                        BDFDB.DataUtils.save(this.settings.notificationSounds, this, "notificationSounds");
                    }
						})
							].filter(n => n)
							...
							children: [
								...
								BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.Button, {
									onClick: _ => {
										...
										if (!source.length) {
											BDFDB.NotificationUtils.toast(`Le fichier sonore a été supprimé.`, {type: "warning"});
											...
											BDFDB.NotificationUtils.toast("Utilisez un lien direct valide vers une source audio ou vidéo, ils se terminent généralement par quelque chose comme .mp3, .mp4 ou .wav", {type: "danger"});
										});
										else BDFDB.LibraryRequires.fs.readFile(source, "base64", (error, body) => {
											if (error) BDFDB.NotificationUtils.toast("Impossible de récupérer le fichier, assurez-vous que le fichier existe", {type: "danger"});
				...
				settingsItems.push(BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.CollapseContainer, {
					title: "Connexion/Déconnexion Timelog",
					collapseStates: collapseStates,
					children: BDFDB.ReactUtils.createElement(BDFDB.LibraryComponents.SettingsItem, {
						type: "Button",
						label: "Vue d'ensemble des connexions/déconnexions de la session actuelle",
